...
const RecipeList = () => {
    const { recipeList, recipePopup, searchRecipeText } = useSelector(selectRecipes);
    const dispatch = useDispatch();

    return (
        <>
            <RecipeListWrapper>
                <Search placeholder="Szukaj przepisu..." onChange={e => dispatch(searchRecipe(e.target.value))} value={searchRecipeText} />
                <RecipeListContainer>
                    {recipeList.length ?
